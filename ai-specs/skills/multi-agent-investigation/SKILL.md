...
- **Frontend-heavy issues** — e.g. flaky effects, hook warnings, or unclear ownership of state vs props: add an `explore` angle against **`lti-frontend-practices`** and the current `frontend/src` tree instead of guessing patterns ad hoc.
...
| Frontend patterns | `explore` | Does the change match `lti-frontend-practices` and the real `frontend/src` layout (hooks, effects, file placement)? |
| Harness layout / docs drift | `explore` | Do `AGENTS.md`, `scripts/`, and `.cursor/*` match the real tree (e.g. **symlink** `.cursor/skills` → `ai-specs/skills`, missing CI, `.gitignore` vs `.env`)? |
